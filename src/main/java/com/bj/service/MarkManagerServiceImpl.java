...
public class MarkManagerServiceImpl implements IMarkManagerService {
    ...
    public boolean addStuMark(int subject_id,String exam_time,String exam_ab, int stu_id,List<Integer> mark) {
        ...
        questionList =  iQuestionDao.queryByExamid(exam_id);//取得所有试题题号
    ...
    @Override
    public List<String> queryByclass(int subject_id,int major_id, String exam_time, String exam_ab, int class_num,int class_inyear) {
        //取总分，求班级平均分，优秀率，及格率、
        List<Integer> marklist = new ArrayList<Integer>();
        List<Student> studentList = new ArrayList<Student>();
        Dclass dclass =iDclassDao.queryBytoId(major_id,class_num,class_inyear);//获得班级id
        Exam exam = iExamDao.queryByExamonly(subject_id,exam_time,exam_ab);//获得试卷id
        if(dclass.getClassId()>0){
            studentList = iStudentDao.queryByClass_id(dclass.getClassId());//查出班级所有学生学号
        }

        for(int i = 0 ; i<studentList.size();i++){
            StuAll stuAll = new StuAll();
            stuAll = iStuAllDao.queryByStuExamid(studentList.get(i).getStuId(),exam.getExamId());//获得第i个学生的成绩
            if(stuAll!=null){
        ...
        int class_avg = 0;//平均分
        int class_good = 0;//优秀率
        int class_pass = 0;//及格率
        ...
        class_good = (int)((double)class_good/(double)studentList.size()*100);//求优秀率
        class_pass = (int)((double)class_pass/(double)studentList.size()*100);//求及格率
        //每道题的全班平均分
        ...
        for(int i = 0 ; i<questionList.size();i++){
            ...
            for(int j = 0;j<studentList.size();j++) {
                StuExam stuExam = new StuExam();
                stuExam = iStuExamDao.queryByQuestionid(questionList.get(i).getQuestionId(), studentList.get(j).getStuId());
                if(stuExam!=null){
